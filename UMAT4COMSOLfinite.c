...
// initialise
  pnewdt=1.0;
  noel=1*par[160];
  coords[0]=1.0*par[161];
  coords[1]=1.0*par[162];
  coords[2]=1.0*par[163];
  time[0]=1.0*states1[150];
  time[1]=1.0*states1[150];
  for (x = 0; x < 3; x++) {for (y = 0; y < 3; y++){dfgrd0[x][y]=1.0*states1[151+3*x+y];};};
//  dt[0]=delta[0]-states1[150];
  dtime[0]=1.0*delta[0];
  if (time[0]==0){dfgrd0[0][0]=1.;dfgrd0[1][1]=1.;dfgrd0[2][2]=1.;};
//  if (time[0]==0){FlOld[0][0]=1.;FlOld[1][1]=1.;FlOld[2][2]=1.;};
...
//  dfgrd0[0][0]=1.*FlOld[0][0];dfgrd0[0][1]=1.*FlOld[1][0];dfgrd0[0][2]=1.*FlOld[2][0];
//  dfgrd0[1][0]=1.*FlOld[0][1];dfgrd0[1][1]=1.*FlOld[1][1];dfgrd0[1][2]=1.*FlOld[2][1];
//  dfgrd0[2][0]=1.*FlOld[0][2];dfgrd0[2][1]=1.*FlOld[1][2];dfgrd0[2][2]=1.*FlOld[2][2];
...
// stran and stress in t
  stre[0][0]=1.0*states1[160];
  stre[1][1]=1.0*states1[161];
  stre[2][2]=1.0*states1[162];
  stre[0][1]=1.0*states1[163];
  stre[1][0]=1.0*states1[163];
  stre[0][2]=1.0*states1[164];
  stre[2][0]=1.0*states1[164];
  stre[1][2]=1.0*states1[165];
  stre[2][1]=1.0*states1[165];
  stra[0][0]=1.0*states1[202];
  stra[1][1]=1.0*states1[203];
  stra[2][2]=1.0*states1[204];
  stra[0][1]=0.5*states1[205];
  stra[1][0]=0.5*states1[205];
  stra[0][2]=0.5*states1[206];
  stra[2][0]=0.5*states1[206];
  stra[1][2]=0.5*states1[207];
  stra[2][1]=0.5*states1[207];
...
// umat
  if(dfgrd0[0][0]==dfgrd1[0][0] && dfgrd0[0][1]==dfgrd1[0][1] && dfgrd0[0][2]==dfgrd1[0][2] &&
   dfgrd0[1][0]==dfgrd1[1][0] && dfgrd0[1][1]==dfgrd1[1][1] && dfgrd0[1][2]==dfgrd1[1][2] &&
   dfgrd0[2][0]==dfgrd1[2][0] && dfgrd0[2][1]==dfgrd1[2][1] && dfgrd0[2][2]==dfgrd1[2][2] && time[0]>0){
     for (x = 0; x < 6; x++) {stress[x]=1.0*states1[160+x];};
     for (x = 0; x < 6; x++) {for (y = 0; y < 6; y++){ddsdde[x][y]=1.0*states1[166+6*x+y];}};}
  else{
...
// save state
  states1[150]=states1[150]+dtime[0];
  for (x = 0; x < 3; x++) {for (y = 0; y < 3; y++){states1[151+3*x+y]=1.0*dfgrd1[x][y];}};
  for (x = 0; x < 6; x++) {states1[160+x]=1.0*stress[x];};
  for (x = 0; x < 6; x++) {for (y = 0; y < 6; y++){states1[166+6*x+y]=1.0*ddsdde[x][y];}};
  for (x = 0; x < 6; x++) {states1[202+x]=stran[x]+dstran[x];};
         ...
         for (k = 0; k < 3; k++){
             ...
             dsdf[i][j][k][l]=0.0;
                 for (p = 0; p < 3; p++){
                     ...
                     sum3[i][p][k][l]=0.0;
                     for (m = 0; m < 3; m++){
                         sum1[i][p][k][l]=sum1[i][p][k][l]+
                          det*ddsdde4[i][p][k][m]*finv[l][m]+
                          0.5*det*Indim[i][k]*finv[l][m]*stre[m][p]+
                          0.5*det*Indim[p][k]*finv[l][m]*stre[i][m];
                         sum2[p][j][k][l]=-det*stre[p][m]*finv[l][m]*finv[j][k];
                         sum3[i][p][k][l]=-det*stre[m][p]*finv[l][m]*finv[i][k];}
                     sum1[i][p][k][l]=sum1[i][p][k][l]-0.5*det*finv[l][i]*stre[k][p]-
                      0.5*det*finv[l][p]*stre[i][k];
                     dsdf[i][j][k][l]=dsdf[i][j][k][l]+sum1[i][p][k][l]*finv[j][p]+
                      finv[i][p]*sum2[p][j][k][l]+sum3[i][p][k][l]*finv[j][p];};};};};};
  for (i = 0; i < 3; i++) {
    for (j = 0; j < 3; j++) {
