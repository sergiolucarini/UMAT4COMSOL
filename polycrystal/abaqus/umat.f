...
C
      IF (NLGEOM.NE.0) THEN
c         if((abs(coords(1)-3.3126416272670d-2).lt.1d-5).and.
c     1    (abs(coords(2)-9.7438277555211190d-3).lt.1d-5).and.
c     2    (abs(coords(3)-1.1993253832032909d-2).lt.1d-5))then
         if((noel.eq.644).and.((abs(coords(2)-4.2d-2)).lt.0.5d-2))then
         write(*,*)'time',noel,time,dtime,coords
         write(*,*)'DROT',DROT
         write(*,*)'F1',dfgrd1
         write(*,*)'F0',dfgrd0
         write(*,*)'S0',stress
...
C
C         CALL GSLPINIT (STATEV(1), NSLIP, NSLPTL, NSET, PROPS(97))
         CALL GSLPINIT (STATEV(1), NSLIP, NSLPTL, NSET, PROPS(73))
